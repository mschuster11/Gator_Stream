...
void wave_parse_header(WaveFile* self) {
    ...
    read_count = fread((uint16_t*)(&self->chunk.format_chunk) + WAVE_RIFF_HEADER_SIZE,
    ...
    if (self->chunk.fact_chunk.id == WAVE_FACT_CHUNK_ID) {
        read_count = fread((uint16_t*)(&self->chunk.fact_chunk) + WAVE_RIFF_HEADER_SIZE,
...
void wave_init(WaveFile* self, char* filename, char* mode) {
    ...
    self->fp = f_open(filename, numericalMode);
    if (self->fp == NULL) {
        self->error_code = WAVE_ERROR_STDIO;
        return;
    }
